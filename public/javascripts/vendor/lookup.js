...
function getDNRServletURL(numberToReplace, format, account, customer, campaign, adsource) {
  ...
  var textcolor = getHex(getStyle(numberToReplace, 'color'));
  var fontfamily = getStyle(numberToReplace, 'font-family');
  if (fontfamily == null) {
    fontfamily = getStyle(numberToReplace, 'fontFamily');
  }
  var fontsize = getStyle(numberToReplace, 'font-size');
  if (fontsize == null) {
    fontsize = getStyle(numberToReplace, 'fontSize');
  }
  var fontstyle = getStyle(numberToReplace, 'font-style');
  if (fontstyle == null) {
    fontstyle = getStyle(numberToReplace, 'fontStyle');
  }
  var fontweight = getStyle(numberToReplace, 'font-weight');
  if (fontweight == null) {
    fontweight = getStyle(numberToReplace, 'fontWeight');
  }
  var servletRequest = 'http://' + callsourceServer +
                       '/simplelookup/Lookup?ctd_ac=' + account +
                       '&ctd_co=' + customer + '&ctx_name=' + campaign +
                       '&ct_Ad%20Source=' + adsource + '&fmt=' + format +
                       '&number=' + $(numberToReplace).text().replace(/[^\d]/g, '') +
                       '&referrer=' + referrerHost + 
                       '&textcolor=' + textcolor + '&fontfamily=' + fontfamily +
                       '&fontsize=' + fontsize + '&fontstyle=' + fontstyle +
                       '&fontweight=' + fontweight;
  servletRequest = servletRequest.replace(/\"/g, "'");

  return servletRequest;
}

function getDNRiFrame(numberToReplace, format, account, customer, campaign, adsource) {
  return '<iframe allowtransparency="true" frameborder=0 marginwidth=0 marginheight=0 scrolling="no" width="' +
               frameWidth + '" height="' + frameHeight + '" src="' +
               getDNRServletURL(numberToReplace, format, account, customer, campaign, adsource) +
               '"></iframe>';
}

function replaceNumber(numberToReplace, format, account, customer, campaign, adsource) {  
  var iframe = getDNRiFrame(numberToReplace, format, account, customer, campaign, adsource); 
