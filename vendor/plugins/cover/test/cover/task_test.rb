...
module Cover
  class TaskTest < Test::Unit::TestCase
    ...
    context "An instance of the Task class" do
      ...
      should "allow additional exclude paths" do
        original_exclude = @task.exclude.dup
        @task.exclude << 'other/path'
        @task.exclude.should == original_exclude + ['other/path']
      end
      
      should "allow multiple additional exclude paths" do
        original_exclude = @task.exclude.dup
        
        @task.exclude << ['other/path', 'another/path']
        @task.exclude.should == original_exclude + [['other/path', 'another/path']]
      end
      ...
      should "know how to generate the options for RCov when there are no duplicate exclude paths" do
      ...
      should "know how to generate the options for RCov when there are duplicate exclude paths" do
        @task.stubs(:exclude).with().returns(['one', 'one'])
      
        options = ['--rails', '--text-report', "-x 'one'"]
      
        @task.options.sort.should == options.sort
      end
      
      should "know how to generate the options for RCov when there are multiple sets of exclude paths" do
        @task.stubs(:exclude).with().returns([['one'], ['two']])
